...
const LandingPage = () => {
  ...
  useEffect(() => {
    posthog.capture('eunice_landing_page_viewed', {
    ...
    const trackScrollDepth = () => {
      ...
      if (scrollPercent > maxScrollDepth) {
        ...
        if (maxScrollDepth === 25 || maxScrollDepth === 50 || maxScrollDepth === 75 || maxScrollDepth === 100) {
          posthog.capture('eunice_scroll_depth_milestone', {
    ...
    return () => {
      ...
      posthog.capture('eunice_final_scroll_depth', {
        depth_percentage: maxScrollDepth,
        page: 'landing_page'
      });
    };
  ...
  const handleButtonClick = () => {
    ...
    posthog.capture('eunice_join_button_clicked', {
      timestamp: new Date().toISOString(),
      is_mobile: isMobile,
    ...
    // Track loading state
    posthog.capture('eunice_loading_screen_started', {
    ...
    const timer = setTimeout(() => {
      ...
      posthog.capture('eunice_navigation_event', {
        from_page: 'landing_page',
        to_page: 'onboarding_page',
      ...
      navigate('/eunice/join', { state: { referrerId } });
      setIsLoading(false);
    }, 3000);
